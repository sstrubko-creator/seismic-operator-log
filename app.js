...
const HISTORY_KEY = 'seismicOperatorLogHistoryV1';
const LEGACY_KEY = 'seismicOperatorLog';
const ACTIVE_ID_KEY = 'seismicOperatorLogActiveId';
let activeLogId = localStorage.getItem(ACTIVE_ID_KEY) || '';

function uid(){ return 'log-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8); }
...
function journalTitle(data){ return String(data.projectName||data.projectCode||'Журнал без названия').trim() || 'Журнал без названия'; }
function upsertHistory(data, opts={}){
  if(!activeLogId) activeLogId=uid();
  localStorage.setItem(ACTIVE_ID_KEY,activeLogId);
  const items=readHistory();
  const now=new Date().toISOString();
  const idx=items.findIndex(x=>x.id===activeLogId);
  const entry={id:activeLogId,title:journalTitle(data),updatedAt:now,createdAt:idx>=0?(items[idx].createdAt||now):now,data};
  if(idx>=0) items[idx]=entry; else items.unshift(entry);
  items.sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)));
  writeHistory(items);
  localStorage.setItem(LEGACY_KEY,JSON.stringify(data));
  if(!opts.silent) showToast('Журнал сохранен');
  return entry;
}
function migrateLegacy(){
  ...
  try{
    ...
    activeLogId=activeLogId||uid();
    localStorage.setItem(ACTIVE_ID_KEY,activeLogId);
    const now=new Date().toISOString();
    writeHistory([{id:activeLogId,title:journalTitle(data),createdAt:now,updatedAt:now,data}]);
  }catch(e){}
}
...
function save(){upsertHistory(collect())}
function newLog(){
  if(!window.confirm('Создать новый журнал? Текущий журнал останется в истории, а форма будет очищена.')) return;
  clearTimeout(window._autoSave);
  const current=collect();
  if(Object.values(current).some(v=>Array.isArray(v)?v.length:String(v||'').trim())) upsertHistory(current,{silent:true});
  activeLogId=uid();
  localStorage.setItem(ACTIVE_ID_KEY,activeLogId);
  form.reset();
  for(const type of ['sources','receivers','crew','log']) document.querySelector(`#${tableMap[type]} tbody`).innerHTML='';
  addRow('sources'); addRow('receivers'); addRow('crew'); for(let i=0;i<5;i++) addRow('log');
  upsertHistory(collect(),{silent:true});
  showStep(0);
  updateProgress();
...
migrateLegacy();
const historyOnStart=readHistory();
let startEntry=activeLogId?historyOnStart.find(x=>x.id===activeLogId):null;
if(!startEntry && historyOnStart.length){startEntry=historyOnStart[0];activeLogId=startEntry.id;localStorage.setItem(ACTIVE_ID_KEY,activeLogId)}
if(startEntry&&startEntry.data){try{restore(startEntry.data)}catch(e){}}
form.addEventListener('input',()=>{updateProgress();clearTimeout(window._autoSave);window._autoSave=setTimeout(()=>upsertHistory(collect(),{silent:true}),500)});


function formatHistoryDate(iso){
  try{return new Intl.DateTimeFormat('ru-RU',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(iso));}
  catch(e){return '';}
}
function renderHistory(){
  ...
  const items=readHistory();
  list.innerHTML='';
  if(!items.length){list.innerHTML='<div class="history-empty">История пока пуста. Заполните и сохраните первый журнал.</div>';return;}
  items.forEach(item=>{
    const d=item.data||{};
    const card=document.createElement('article'); card.className='history-card'+(item.id===activeLogId?' active':'');
    const info=document.createElement('button'); info.type='button'; info.className='history-open';
    const sub=[d.projectCode,d.client].filter(Boolean).join(' · ');
    info.innerHTML=`<strong>${escapeHtml(item.title||'Журнал без названия')}</strong><span>${escapeHtml(sub||'Без шифра и заказчика')}</span><small>Изменен: ${escapeHtml(formatHistoryDate(item.updatedAt))}</small>`;
    info.onclick=()=>openHistoryItem(item.id);
    const del=document.createElement('button'); del.type='button'; del.className='history-delete'; del.textContent='Удалить'; del.onclick=(e)=>{e.stopPropagation();deleteHistoryItem(item.id)};
    card.append(info,del); list.append(card);
  });
}
function escapeHtml(v){return String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));}
function openHistory(){renderHistory();const m=document.getElementById('historyModal');m.classList.add('show');m.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');}
function closeHistory(){const m=document.getElementById('historyModal');m.classList.remove('show');m.setAttribute('aria-hidden','true');document.body.classList.remove('modal-open');}
function openHistoryItem(id){
  const item=readHistory().find(x=>x.id===id); if(!item) return;
  clearTimeout(window._autoSave);
  activeLogId=id; localStorage.setItem(ACTIVE_ID_KEY,id);
  restore(item.data||{}); localStorage.setItem(LEGACY_KEY,JSON.stringify(item.data||{}));
  closeHistory(); showStep(0); showToast(`Открыт: ${item.title||'журнал'}`);
}
function deleteHistoryItem(id){
  const item=readHistory().find(x=>x.id===id); if(!item) return;
  if(!window.confirm(`Удалить журнал «${item.title||'Без названия'}» из истории?`)) return;
  let items=readHistory().filter(x=>x.id!==id); writeHistory(items);
  if(id===activeLogId){
    if(items.length){activeLogId=items[0].id;localStorage.setItem(ACTIVE_ID_KEY,activeLogId);restore(items[0].data||{});localStorage.setItem(LEGACY_KEY,JSON.stringify(items[0].data||{}));}
    else{activeLogId=uid();localStorage.setItem(ACTIVE_ID_KEY,activeLogId);localStorage.removeItem(LEGACY_KEY);form.reset();for(const type of ['sources','receivers','crew','log']){document.querySelector(`#${tableMap[type]} tbody`).innerHTML='';addRow(type)};}
  }
  renderHistory(); showToast('Журнал удален из истории');
}
document.getElementById('historyBtn').onclick=openHistory;
document.getElementById('historyClose').onclick=closeHistory;
document.querySelector('[data-history-close]').onclick=closeHistory;
...
fileMenuBtn.onclick=(e)=>{e.stopPropagation();const opening=fileMenuList.hidden;fileMenuList.hidden=!opening;fileMenuBtn.setAttribute('aria-expanded',opening?'true':'false')};
fileMenuList.addEventListener('click',()=>setTimeout(closeFileMenu,0));
document.addEventListener('click',(e)=>{if(!fileMenu.contains(e.target))closeFileMenu()});

// Feedback form (no backend): prepares a mailto draft in the user's mail app.
const feedbackModal=document.getElementById('feedbackModal');
function openFeedback(){feedbackModal.classList.add('show');feedbackModal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');setTimeout(()=>document.getElementById('feedbackMessage').focus(),20)}
function closeFeedback(){feedbackModal.classList.remove('show');feedbackModal.setAttribute('aria-hidden','true');document.body.classList.remove('modal-open')}
...
document.querySelector('[data-feedback-close]').onclick=closeFeedback;
document.getElementById('feedbackSend').onclick=()=>{
  const name=document.getElementById('feedbackName').value.trim();
  const contact=document.getElementById('feedbackContact').value.trim();
  const message=document.getElementById('feedbackMessage').value.trim();
  if(!message){showToast('Введите комментарий');document.getElementById('feedbackMessage').focus();return}
  const d=collect();
  const subject=`Обратная связь — ${d.projectName||'Сейсможурнал'}`;
  const body=[
    'Обратная связь по приложению «Журнал оператора сейсморазведки»',
    '',
    `Имя: ${name||'не указано'}`,
    `Контакт: ${contact||'не указан'}`,
    `Проект: ${d.projectName||'не указан'}`,
    `Шифр проекта: ${d.projectCode||'не указан'}`,
    '',
    'Комментарий:',
    message
  ].join('\n');
  const url=`mailto:sstrubko@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  window.location.href=url;
  closeFeedback();
};

// Large comment editor for operator log rows
let activeCommentField=null;
const commentModal=document.getElementById('commentModal');
const commentEditor=document.getElementById('commentEditor');
function openCommentEditor(field){activeCommentField=field;commentEditor.value=field.value||'';commentModal.classList.add('show');commentModal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');setTimeout(()=>{commentEditor.focus();commentEditor.setSelectionRange(commentEditor.value.length,commentEditor.value.length)},20)}
function closeCommentEditor(commit=true){if(commit&&activeCommentField){activeCommentField.value=commentEditor.value;activeCommentField.dispatchEvent(new Event('input',{bubbles:true}))}commentModal.classList.remove('show');commentModal.setAttribute('aria-hidden','true');document.body.classList.remove('modal-open');activeCommentField=null}
document.getElementById('logTable').addEventListener('click',(e)=>{const f=e.target.closest('textarea[data-k="comment"]');if(f)openCommentEditor(f)});
document.getElementById('commentDone').onclick=()=>closeCommentEditor(true);
document.getElementById('commentClose').onclick=()=>closeCommentEditor(true);
document.querySelector('[data-comment-close]').onclick=()=>closeCommentEditor(true);
...
document.getElementById('demoBtn').onclick=()=>{restore(demo);activeLogId=uid();localStorage.setItem(ACTIVE_ID_KEY,activeLogId);upsertHistory(collect(),{silent:true});showToast('Загружены демонстрационные данные')};
